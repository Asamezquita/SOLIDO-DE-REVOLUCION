...
# Definir las nuevas funciones en sus respectivos intervalos
def g(x):
    if 0 <= x <= 0.45:
...
def p(x):
    if 0.44 <= x <= 2.06:
        return (-0.08 * x**6 + 0.04 * x**5 + 1.78 * x**4 -
                5.04 * x**3 + 4.25 * x**2 - 0.88 * x + 1.79)
    return np.nan
...
def q_3(x):
    if 5.58 <= x <= 5.93:
        return (368.32 * x**5 - 10447.75 * x**4 + 118484.82 * x**3 -
                671507.35 * x**2 + 1901876.49 * x - 2153486.97)
    return np.nan
...
def area_under_curve(func, a, b):
    # Calcula el área usando la integral
